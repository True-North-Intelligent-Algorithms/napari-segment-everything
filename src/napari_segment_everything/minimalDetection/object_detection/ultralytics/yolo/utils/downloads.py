...
from ..utils import LOGGER, checks, clean_url, emojis, is_online, url2file
...
def unzip_file(file, path=None, exclude=(".DS_Store", "__MACOSX")):
    ...
    with ZipFile(file) as zipObj:
        file_list = [
            f for f in zipObj.namelist() if all(x not in f for x in exclude)
        ]
...
def check_disk_space(
    ...
    with contextlib.suppress(Exception):
        ...
        data = (
            int(requests.head(url).headers["Content-Length"]) / gib
        )  # file size (GB)
...
def safe_download(
    ...
    if unzip and f.exists() and f.suffix in ("", ".zip", ".tar", ".gz"):
        unzip_dir = (
            dir or f.parent
        )  # unzip to dir if provided else unzip in place
